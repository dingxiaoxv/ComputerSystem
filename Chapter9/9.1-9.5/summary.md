...
补充：`SwapCached`（`/proc/meminfo`）= 换出过、又被读回内存但 swap 里副本仍保留的页，类似"文件干净页"——再次换出可免写直接丢。

**🎯 zram / zswap：拿内存压缩页当 swap，为什么「占内存反而省内存」**

直觉上「划一块内存当 swap」应该让可用内存变少，但 zram/zswap 实际净效果通常是**可用内存变多**——因为它不是静态预留，而是**动态 + 压缩**：只在冷匿名页被换出时按压缩后大小占内存，用 CPU 把冷页体积压到约 1/3。先分清两者机制不同：

- **zram**：一个压缩的 RAM 块设备挂上去当 swap，**无磁盘后端**。按需分配——有匿名页换出才申请内存存压缩数据，没换出时占用近乎 0。换入只需解压（μs 级），不碰盘
- **zswap**：不是独立 swap，而是**真实磁盘 swap 前面的一层压缩缓存**。页换出时先压缩塞进内存池（`max_pool_percent` 默认上限 20%），池满了再把最老的压缩页**写穿到磁盘 swap**。所以 zswap 必须配一个后端磁盘 swap

为什么省内存——核心是匿名页内容高度可压缩（典型 2:1~4:1）。同一批 4GB **冷**匿名页（唯一副本、不能丢）：

| 方案 | 这 4GB 冷页占的物理内存 | 代价 |
|------|------|------|
| 无 swap | 4GB（赖在内存不可驱逐） | 内存一紧张直接 OOM |
| 传统磁盘 swap | ~0（全换到磁盘） | 换入是 major fault，ms 级 |
| zram（3:1） | ~1.3GB（压缩留在内存） | 换入只需解压，μs 级 |

对比一三行：同批冷页从占 4GB 变 1.3GB，等于多腾出 2.7GB 给工作集——没创造内存，而是**把冷页压扁了**。对比二三行：zram 多占 1.3GB 但省掉磁盘 I/O，是「CPU/内存 换 延迟」。

**⚠️ 三个关键点别搞反**

- **只压冷页**：进 zram/zswap 的是 `vm.swappiness` 从 LRU 尾部挑出的冷页，**热的工作集永远不压**、全速访问。所以「占内存」只发生在本就要被驱逐的冷页上
- **CPU 是成本**：每次换出压缩、换入解压。算法选 `lz4`/`lzo`（快、比低）还是 `zstd`（慢、比高）就是在调 CPU↔压缩比；CPU 有余量、延迟敏感的场景（云主机、Android、嵌入式）很划算
- **zram 无兜底、zswap 有**：zram 遇到压不动的页（已加密/已压缩数据，压缩比 <1）会原样占内存、极端仍可能 OOM；zswap 池满写穿到磁盘，更稳
- **两者别同时开（互斥替代，不是互补）**：zswap 是「swap 设备**前面**」的压缩缓存，而 zram 本身**就是一个 swap 设备**。同时开时换出路径变成 `冷页 → zswap 压缩存池 → 池满写穿到后端 swap(=zram) → zram 把已压缩数据再压一次`——第二次对已压数据压缩比≈1，纯烧 CPU 还多一层管理开销。所以按「有没有磁盘 swap」二选一：无磁盘后端用 zram，有磁盘 swap 想吸收换出突发用 zswap

```bash
zramctl                          # 看 zram 设备的算法、原始数据量 vs 压缩后占用（DATA/COMPR）
...
> 这里只列了页表数据结构。想看缺页、demand paging、COW、rmap、页回收、脏页回写的**完整函数调用链与代码路径**，见内核虚拟内存说明书 [`../9.13-kernel-vm-source/00-index.md`](../9.13-kernel-vm-source/00-index.md)（含「零页别名惩罚」遗留问题的代码级解答）。
