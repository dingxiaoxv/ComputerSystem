...
🔧 **先立靶子：page cache（页缓存）是 Linux 给 §9.3「DRAM 缓存磁盘」的工程落地**
本场景所有细节（两次拷贝、写后立即读、脏页、并发写）都围绕同一个对象——**page cache**。它是内核拿空闲物理内存缓存**文件内容**的那部分：`read`/`write`/`mmap` 一个文件，数据都先进 page cache，**按 `(inode, 页内偏移)` 索引、全机一份**。CSAPP §9.3 把虚拟内存抽象成「用 DRAM 缓存磁盘」（页命中 vs 缺页换入），page cache 就是这个抽象**在文件这条线上的实现**；注意它不覆盖匿名页（堆/栈没有文件后备，走 swap 而非 page cache）。

```
                       ┌──────────── page cache（内核空闲内存，按 inode+offset 索引）────────────┐
  read/write/mmap 文件 │   file 各页 = 物理页帧（cached）   ←─ 命中：纳秒级，不碰盘            │
        ────────────▶ │   未缓存的页（uncached）          ←─ 缺页 major fault：从磁盘读入     │
                       │   被改过的页（dirty）             ──▶ 异步回写磁盘（仅为持久化）       │
                       └──────────────────────────────────────────────────────────────────────┘
   匿名页（堆/栈）不在这里：无文件后备 → 内存不足时换出到 swap（受 vm.swappiness）
```

**它代表什么（为什么要掌握）**：① **文件 I/O 性能的底座**——命中就不碰磁盘，「冷启动慢、第二次快」就是第二次文件页已 cached；② **可回收内存**——page cache 干净页随时能丢，所以 `buff/cache` 高是好事不是吃满，`MemAvailable` 因此远大于 `MemFree`；③ **跨进程一致 + 脏页**——多进程经同一份缓存页通信（写后立即读），改了没回写的就是脏页（断电会丢、也是回写 I/O 风暴来源）；④ 进程 `smaps` 里 file-backed 的 `Rss`，就是「这个进程映射到、且已 cached 的文件页」。

```bash
# 系统级：buff/cache 就是页缓存总量；meminfo 看细分
free -h
grep -E '^(Cached|Buffers|Dirty|Writeback|Active\(file\)|Mapped):' /proc/meminfo

# 文件级：某文件有多少页在缓存里
fincore bigfile            # util-linux 自带，列出该文件 RES/PAGES 缓存量
vmtouch -v bigfile         # 逐页 0/1 + 缓存百分比（需另装）；程序内精确查用 mincore(2)

# 进程级：smaps 里带文件路径的映射区，Rss 即其缓存驻留量
grep -A4 '\.so' /proc/<pid>/smaps | grep -E 'Rss|Referenced'

# 验证：清空页缓存后再访问，命中变 major fault
...
🔗 **串到的小节**：§9.3 VM 作为缓存工具（page cache 是「DRAM 缓存磁盘」抽象的文件线落地、cached/uncached、页命中 vs major fault）、§9.8.4 mmap（read vs mmap 的拷贝差异）、§9.8.1 共享对象 vs 私有对象（SHARED 回盘 / PRIVATE COW）、§9.1-9.5「脏页」专题（回写四条路、`_PAGE_DIRTY` 硬件置位）、§9.3 major vs minor fault、§10.8 共享文件（open file description / offset 共享、`O_APPEND` 原子追加、文件锁）。
